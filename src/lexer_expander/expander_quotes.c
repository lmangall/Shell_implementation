...
char *erase_outside_quotes(const char *str)
{
    ...
    char *new_str = (char *)malloc((ft_strlen(str) + 1) * sizeof(char));
    if (new_str == NULL)
    {
    ...
    // No need to free the original string here
...
int quote_type(const char *str)
{
    ...
    while (*str)
    {
        if (*str == '\"')
        {
            ...
            else if (single && ft_strrchr(str + 1, '\"') != NULL && ft_strchr(str + 1, '\'') != NULL)
        ...
        else if (*str == '\'')
        {
            ...
            else if (doubleq && ft_strrchr(str + 1, '\'') != NULL && ft_strchr(str + 1, '\"') != NULL)
                return 2;
            else
                return 4;
        }
