...
int	set_var(t_data *data, const char *name, const char *value)
{
	...
	while (i < data->num_vars)
	{
		...
		{
			strncpy(data->vc[i].value, value,
				sizeof(data->vc[i].value) - 1);
			data->vc[i].value[sizeof(data->vc[i].value)
				- 1] = '\0';
			return (0);
		}
		i++;
...
int set_shell_var(t_data *data, const char *name, const char *value)
{
    ...
    if (data->num_shell_vars < MAX_VARS)
    {
        ...
        data->shell_vc[data->num_shell_vars].name[sizeof(data->shell_vc[data->num_shell_vars].name) - 1] = '\0';
        strncpy(data->shell_vc[data->num_shell_vars].value, value,
                sizeof(data->shell_vc[data->num_shell_vars].value) - 1);
        data->shell_vc[data->num_shell_vars].value[sizeof(data->shell_vc[data->num_shell_vars].value) - 1] = '\0';
        data->num_shell_vars++;
        return (0);
    }
    else
